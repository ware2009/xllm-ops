...
"""LightningIndexer V2 metadata stage between input restoration and the main API."""
...
def clone_metadata_value(value):
    ...
    if hasattr(value, "item"):
        return value.item()
    raise TypeError(
        f"unsupported LightningIndexer V2 metadata input type: {type(value).__name__}"
    )
...
def persist_metadata_inputs(testcase_name, metadata_input, context):
    ...
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise ValueError(
            f"LightningIndexer V2 metadata state is not a regular file: {path}"
        )
...
def load_metadata_inputs(context):
    ...
    if (
        ...
        raise ValueError(
            "LightningIndexer V2 metadata state has an incompatible schema"
        )
    if payload.get("csv_contract") != csv_contract_digest(context):
        raise ValueError(
            "LightningIndexer V2 metadata state does not match the current CSV contract"
        )
    metadata_input = payload.get("metadata_input")
    if not isinstance(metadata_input, dict):
        raise ValueError("LightningIndexer V2 metadata state lacks metadata_input")
    context.state[METADATA_INPUTS_STATE_KEY] = payload
...
def metadata_arguments(context):
    ...
    if metadata_input is None:
        raise RuntimeError(
            "LightningIndexer V2 requires input-stage metadata inputs before pre-NPU"
        )
    return metadata_input


def parse_metadata_only_option(kwargs):
    """Keep absent, explicit false, and explicit true as three separate states."""
...
def copy_metadata(target, source):
    ...
    if tuple(target.shape) != tuple(source.shape):
        raise ValueError(
            f"LI_V2 metadata shape mismatch: placeholder={tuple(target.shape)}, "
...
def persist_metadata(context, metadata):
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"LI_V2 expected one stored metadata input, found {len(matches)} in {case_dir}"
        )
    path = matches[0]
    temporary = path.with_name(f".{path.name}.{os.getpid()}.tmp")
    array = (
    ...
    if file_format == "bin":
        ...
        raise ValueError(f"LI_V2 cannot persist manual-data format {file_format!r}")
    temporary.replace(path)
...
def run_torch(context, target, arguments):
    ...
    def operation():
        nonlocal generated
        generated = torch.ops.cann_ops_transformer.lightning_indexer_metadata(
    ...
    with path.open("a+", newline="", encoding="utf-8") as file:
        ...
        try:
            ...
            if header is not None and tuple(header) != METADATA_RESULT_FIELDS:
                raise RuntimeError(
                    f"LI_V2 metadata result header does not match: {path}"
                )
...
def run(*args, context: TtkContext = None, **kwargs):
    """Generate/reuse metadata according to the three-state stop attribute."""
    del args
    if context is None:
        raise RuntimeError(
            "LI_V2 metadata stage requires an explicitly injected context"
        )
    ...
    try:
        ...
        if len(tensors) <= METADATA_INDEX or tensors[METADATA_INDEX] is None:
            raise ValueError("LI_V2 CSV must reserve tensor slot 10 for metadata")
        ...
        if context.case_type == "aclnn":
            ...
            raise ValueError(
                f"LI_V2 pre-NPU does not support case type {context.case_type!r}"
            )
        ...
        if stop_after_stage is not None:
            try:
                ...
                logging.exception("Failed to write LI_V2 metadata error result")
        raise

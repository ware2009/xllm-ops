#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_XA_UNSHARED_QK_TLA_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_XA_UNSHARED_QK_TLA_HPP
...
public:
    ...
    static constexpr uint32_t L0C_TILE_SIZE = L0_TILE_M * L1_TILE_N * sizeof(ElementAccumulator);
    ...
    static_assert(
        ...
        "L1 and L0 tile shapes must match on the M and K axes");
    static_assert(
        L1_TILE_N >= L0_TILE_N && L1_TILE_N % L0_TILE_N == 0,
        "The L1 N tile must be an integer multiple of the L0 N tile");
    ...
    static_assert(BLOCK_L0C_SIZE <= ArchTag::L0C_SIZE, "L0C tile buffers exceeding the L0C space!");
...
public:
    ...
    BlockMmadTla(Arch::Resource<ArchTag>& resource, uint32_t& l1BufAddrStart, uint32_t& l0CBufAddrStart)
    {
        for (uint32_t i = 0; i < STAGES; ++i) {
            l1ATensorList_[i] = resource.l1Buf.template GetBufferByByte<ElementA>(l1BufAddrStart);
            l1BufAddrStart += L1A_TILE_SIZE;
            ...
            l0CTensorList_[i] =
                resource.l0CBuf.template GetBufferByByte<ElementAccumulator>(l0CBufAddrStart + L0C_TILE_SIZE * i);
    ...
    CATLASS_DEVICE void operator()(
        ...
        uint64_t QK_UB_RELEASE_FLAG,
        uint32_t& taskIdL0A, uint32_t& taskIdL0B, uint32_t& taskIdL0C)
    {
        uint32_t qActual = tla::get<0>(actualShape);
        uint32_t kActual = tla::get<1>(actualShape);
        uint32_t dimActual = tla::get<2>(actualShape);
        
        auto layoutAInL1 = tla::MakeLayout<ElementA, LayoutTagL1A>(qActual, dimActual);
        auto tensorAInL1 = tla::MakeTensor(l1ATensorList_[l1AListId_], layoutAInL1, Arch::PositionL1{});
        ...
        copyGmToL1A(tensorAInL1, tensorA);
        ...
        auto layoutBInL1 = tla::MakeLayout<ElementB, LayoutTagL1B>(dimActual, kActual);
        ...
        // AscendC::printf("unshared qk taskIdL0A %d taskIdL0B %d eventL0A %d eventL0B %d\n",
        // taskIdL0A, taskIdL0B, l0AEventList_[taskIdL0A], l0BEventList_[taskIdL0B]);
        auto layoutAInL0 = tla::MakeLayout<ElementA, LayoutTagL0A>(qActual, dimActual);
        ...
        auto tensorAL1Tile = GetTile(
            tensorAInL1,
            tla::MakeCoord(0, 0),
            tla::MakeShape(qActual, dimActual)
        );

        // AscendC::printf("qActual %d kActual %d dimActual %d\n", qActual, kActual, dimActual);
        copyL1ToL0A_(tensorAInL0, tensorAL1Tile);

        AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(l1AEventList_[l1AListId_]);
        l1AListId_ = 1 - l1AListId_;
        ...
        auto layoutInL0C = tla::MakeLayoutL0C(qActual, kActual);
        auto tensorInL0C =
            tla::MakeTensor(l0CTensorList_[taskIdL0C], layoutInL0C, Arch::PositionL0C{});

        uint32_t nLoops = (kActual + L0_TILE_N - 1) / L0_TILE_N;
        for (uint32_t nIdx = 0; nIdx < nLoops; ++nIdx) {
            uint32_t nOffset = nIdx * L0_TILE_N;
            uint32_t tileN = nIdx + 1 == nLoops ? kActual - nOffset : L0_TILE_N;

            auto layoutBInL0 = tla::MakeLayout<ElementB, LayoutTagL0B>(dimActual, tileN);
            auto tensorBInL0 = tla::MakeTensor(l0BTensorList_[taskIdL0B], layoutBInL0, Arch::PositionL0B{});
            auto tensorBTileInL1 = GetTile(
                tensorBInL1, tla::MakeCoord(0, nOffset), tla::MakeShape(dimActual, tileN));
            ...
            auto tensorTileInL0C = GetTile(
                tensorInL0C, tla::MakeCoord(0, nOffset), tla::MakeShape(qActual, tileN));
            tileMmad_(tensorTileInL0C, tensorAInL0, tensorBInL0, qActual, tileN, dimActual, true);
        ...
        // AscendC::printf("wait qk_ub_flag %d\n", QK_UB_RELEASE_FLAG);
        AscendC::CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(QK_UB_RELEASE_FLAG);
        AscendC::CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(QK_UB_RELEASE_FLAG + 16);
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_XA_UNSHARED_QK_TLA_HPP

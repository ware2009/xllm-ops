...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_XA_SHARED_QK_TLA_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_XA_SHARED_QK_TLA_HPP
...
public:
    ...
    CATLASS_DEVICE void operator()(
        ...
        uint64_t QK_UB_RELEASE_FLAG, bool isFirstKv, bool isLastKv,
        uint32_t &taskIdL0A, uint32_t &taskIdL0B)
    {
        ...
        copyL1ToL0A(tensorAInL0, tensorAL1Tile);
        AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0AEventList_[taskIdL0A]);

        auto layoutBInL0 = tla::MakeLayout<ElementB, LayoutTagL0B>(dimActual, kActual);
        ...
        auto tensorBL1Tile = GetTile(
            tensorBInL1,
            tla::MakeCoord(0, 0),
            tla::MakeShape(dimActual, kActual)
        );
        ...
        // MTE1 has consumed the L1 tiles, so their buffers can be reused by MTE2.
        ...
        tileMmad(tensorInL0C, tensorAInL0, tensorBInL0, qActual, kActual, dimActual, true);
        ...
        // The two AIVs must both release the shared UB tile before FixPipe overwrites it.
        AscendC::CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(QK_UB_RELEASE_FLAG);
        AscendC::CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(QK_UB_RELEASE_FLAG + AIV1_EVENT_OFFSET);

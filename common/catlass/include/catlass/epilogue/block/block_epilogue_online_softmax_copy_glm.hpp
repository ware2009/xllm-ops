 ...
 */
 ...
 #ifndef CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_ONLINE_SOFTMAX_COPY_SUM_MAX_HPP
 #define CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_ONLINE_SOFTMAX_COPY_SUM_MAX_HPP
 ...
 template <
     ...
     class MaskType_>
 class BlockEpilogue<
     EpilogueAtlasA2OnlineSoftmaxCopySumMax,
     OutputType_,
     InputType_,
     MaskType_>
 {
 public:
     using DispatchPolicy = EpilogueAtlasA2OnlineSoftmaxCopySumMax;
     using ArchTag = typename DispatchPolicy::ArchTag;
     using ElementOutput = typename OutputType_::Element;
     using ElementInput = typename InputType_::Element;
     using ElementMask = typename MaskType_::Element;
     using ElementUpdate = typename InputType_::Element;
     ...
     void SubCoreCompute(AscendC::GlobalTensor<ElementOutput> gOutput, 
                        AscendC::GlobalTensor<ElementUpdate> gSharedMax,
                        AscendC::GlobalTensor<ElementUpdate> gSharedSum,
                         const LayoutOutput &layoutOutput,
                         uint32_t rowOffset, uint32_t isFirstStackTile,
                         uint32_t columnNumRound, uint32_t pingpongFlag,
                         uint32_t curStackTileMod, uint32_t isLastStackTile, uint32_t headNum)
     {
     ...
     void operator()(AscendC::GlobalTensor<ElementOutput> gOutput, AscendC::GlobalTensor<ElementInput> gInput,
                    AscendC::GlobalTensor<ElementUpdate> gSharedMax, AscendC::GlobalTensor<ElementUpdate> gSharedSum, 
                     const LayoutOutput &layoutOutput, const LayoutInput &layoutInput, GemmCoord actualBlockShape,
                     uint32_t isFirstStackTile,  uint32_t isLastStackTile, uint32_t qSBlockSize, uint32_t qNBlockSize, uint32_t curStackTileMod,
                    uint32_t headNum)
     {
         ...
         uint32_t maxRowNumPerLoop = MAX_UB_S_ELEM_NUM / columnNumRound;  // 8192 / 128 = 64
         uint32_t rowNumTile = RoundDown(maxRowNumPerLoop, FLOAT_BLOCK_SIZE); // 64
 ...
 template <
     ...
     class MaskType_>
 class BlockEpilogue<
     EpilogueAscend950OnlineSoftmaxCopySumMax,
     OutputType_,
     InputType_,
     MaskType_>
     : public BlockEpilogue<EpilogueAtlasA2OnlineSoftmaxCopySumMax, OutputType_, InputType_, MaskType_> {
 public:
     using Base = BlockEpilogue<EpilogueAtlasA2OnlineSoftmaxCopySumMax, OutputType_, InputType_, MaskType_>;
     using DispatchPolicy = EpilogueAscend950OnlineSoftmaxCopySumMax;
     using ArchTag = typename DispatchPolicy::ArchTag;
     using Base::Base;
 };
 } // namespace Catlass::Epilogue::Block
 
 #endif // CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_ONLINE_SOFTMAX_COPY_SUM_MAX_HPP
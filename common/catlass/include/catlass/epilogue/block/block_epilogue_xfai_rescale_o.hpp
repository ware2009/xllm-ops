...
#ifndef CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_XFAI_RESCALE_O_HPP
#define CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_XFAI_RESCALE_O_HPP
...
template <
    ...
    LseMode LSE_MODE_>
class BlockEpilogue<
    EpilogueAtlasA2XFAIRescaleO<LSE_MODE_>,
    OutputType_,
    InputType_,
    UpdateType_,
    LseType_>
...
public:
    // Type aliases
    using DispatchPolicy = EpilogueAtlasA2XFAIRescaleO<LSE_MODE_>;
        ...
        uint32_t epiTokenNum, uint32_t integralHeadNum, uint32_t qSThisSubBlock, uint32_t embed, uint32_t oHiddenSize)
    {
        ...
        if (proTokenNum != 0) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset + proTokenIdx * oHiddenSize],
                goUbTensor16[innerGOUbOffset],
                AscendC::DataCopyExtParams(proTokenNum, embed * 2, 0, (oHiddenSize - embed) * 2, 0));
        ...
        for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset],
                goUbTensor16[innerGOUbOffset],
                AscendC::DataCopyExtParams(qSThisSubBlock, embed * 2, 0, (oHiddenSize - embed) * 2, 0));
            innerOGmOffset += embed;
        ...
        if (epiTokenNum != 0) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset],
                goUbTensor16[innerGOUbOffset],
                AscendC::DataCopyExtParams(epiTokenNum, embed * 2, 0, (oHiddenSize - embed) * 2, 0));
        }
    }
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
        ...
        uint32_t qSThisSubBlock = (qNBlockSize == 1) ? inRowActualThisSubBlock : qSBlockSize;
        int64_t outOffsetSubBlock =
            layoutOutput.GetOffset(MatrixCoord(outRowOffsetThisSubBlock, outColOffsetThisSubBlock));

        uint32_t outLseRowOffsetThisSubBlock = (qNBlockSize == 1) ?
            inRowOffsetThisSubBlock : 0;
        uint32_t outLseColOffsetThisSubBlock = (qNBlockSize == 1) ?
        ...
        if (inRowActualThisSubBlock > 0) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                uint32_t rowActualCurLoop =
                    (rowLoopIdx == (rowLoop - 1)) ? inRowActualThisSubBlock - rowLoopIdx * rowTile : rowTile;

                int64_t offsetOutput = rowLoopIdx * rowTile / qSThisSubBlock * embed + outOffsetSubBlock;
...
#endif // CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_XFAI_RESCALE_O_HPP
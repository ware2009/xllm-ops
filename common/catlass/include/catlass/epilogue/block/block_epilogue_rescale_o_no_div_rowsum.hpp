 ...
 */
 ...
 #ifndef CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_NO_DIV_ROWSUM_HPP
 #define CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_NO_DIV_ROWSUM_HPP
 ...
 template <
     ...
     class UpdateType_>
 class BlockEpilogue<
     EpilogueAtlasA2RescaleOWithoutDivSum,
     OutputType_,  // fp16
     InputType_,   // fp32 
     UpdateType_>  // fp32
 {
 public:
     // Type aliases
     using DispatchPolicy = EpilogueAtlasA2RescaleOWithoutDivSum;
         ...
         uint32_t embedRound, uint32_t qNThisSubBlock, uint32_t oHiddenSize)
     {
         if (qNThisSubBlock == 0) {
             AscendC::DataCopyPad(
                 gSharedOut, 
                 goUbTensor32,
                 AscendC::DataCopyExtParams(curRowNum, embed * 4, 0, (oHiddenSize - embed) * 4, 0));
         } else {
             for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                 AscendC::DataCopyPad(
                     gSharedOut[qNIdx * embed],
                     goUbTensor32[qNIdx * embedRound * qSBlockSize],
                     AscendC::DataCopyExtParams(qSBlockSize, embed * 4, 0, (oHiddenSize - embed) * 4, 0));
             }
         }
     }
         ...
         uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
     {
         ...
         if (inRowActualThisSubBlock > 0) {
             ...
             auto gSharedOutThisSubBlock = gSharedOut[offsetOutput];
 ...
 #endif // CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_NO_DIV_ROWSUM_HPP
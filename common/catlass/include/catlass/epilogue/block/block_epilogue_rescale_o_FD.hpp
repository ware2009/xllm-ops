...
#ifndef CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_HPP_FD
#define CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_HPP_FD
...
template <
    ...
    LseMode LSE_MODE_>
class BlockEpilogue<
    EpilogueAtlasA2RescaleO_FD<LSE_MODE_>,
    OutputType_,
    InputType_,
    UpdateType_,
    LseType_>
...
public:
    // Type aliases
    using DispatchPolicy = EpilogueAtlasA2RescaleO_FD<LSE_MODE_>;
        ...
        uint32_t epiTokenNum, uint32_t integralHeadNum, uint32_t qSThisSubBlock, uint32_t embed, uint32_t oHiddenSize)
    {
        ...
        __aicore__ inline
    void CopyOToGmFp32(AscendC::GlobalTensor<float> gOutput,
        uint32_t proTokenIdx, uint32_t proTokenNum,
        uint32_t epiTokenNum, uint32_t integralHeadNum,
        uint32_t qSThisSubBlock, uint32_t embed, uint32_t oHiddenSize, uint32_t oHiddenSize_gmlo)
    {
        uint32_t innerOGmOffset = 0;
        uint32_t innerGOUbOffset = 0;
        ...
        if (proTokenNum != 0U) {
            AscendC::DataCopyPad(
                ...
                AscendC::DataCopyExtParams(
                    proTokenNum,
                    embed * sizeof(float),                      // 每行有效 embed 列，float
                    0,
                    (oHiddenSize_gmlo - embed) * sizeof(float),      // 每行 padding
                    0));
            innerOGmOffset += embed;
            innerGOUbOffset += proTokenNum * embed;
        }
        ...
        for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
            AscendC::DataCopyPad(
                ...
                AscendC::DataCopyExtParams(
                    ...
                    embed * sizeof(float),
                    0,
                    (oHiddenSize_gmlo - embed) * sizeof(float),
                    0));
            innerOGmOffset += embed;
            innerGOUbOffset += qSThisSubBlock * embed;
        }
    ...
    void SubCoreCompute(
        ...
        uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum, uint32_t integralHeadNum, bool isSplitkv, const LayoutOutput &layoutgmLse, 
        const LayoutInput &layoutgmLo)
    {
        ...
        uint32_t oHiddenSize_gmlo = layoutgmLo.shape(1);
        uint32_t qHeads_gmlse = layoutgmLse.shape(1);
        ...
        if (isLastStackTile) {
            ...
            if (isSplitkv) {
                CopyOToGmFp32(
                    gCombineo,                 // 注意：这里的 gCombineo 需要是 GlobalTensor<float>
                    proTokenIdx,
                    proTokenNum,
                    epiTokenNum,
                    integralHeadNum,
    ...
    void operator()(
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod, bool isSplitkv, const LayoutLse &layoutgmLse, 
        const LayoutInput &layoutgmLo)
    {
        ...
        int64_t gmlooutOffsetSubBlock = 
            layoutgmLo.GetOffset(MatrixCoord(outRowOffsetThisSubBlock, outColOffsetThisSubBlock));
        ...
        int64_t gmLseoffsetLse = 
            layoutgmLse.GetOffset(MatrixCoord(outLseRowOffsetThisSubBlock, outLseColOffsetThisSubBlock));

        auto gCombineLseSubBlock = gCombineLse[gmLseoffsetLse];
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                auto gCombineoSubBlock = gCombineo[gmloffset];
...
#endif // CATLASS_EPILOGUE_BLOCK_BLOCK_EPILOGUE_RESCALE_O_HPP_FD
